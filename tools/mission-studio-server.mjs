...
async function handler(req,res){
  try {
    if(req.method==='GET' && req.url==='/') { const html=await fs.readFile(studio,'utf8'); res.writeHead(200,{'Content-Type':'text/html; charset=utf-8'}); return res.end(html); }
    ...
    if(req.method==='POST' && req.url==='/api/save-all') {
      const body=await readBody(req); const lang=body.lang==='en'?'en':'ru'; const mission=body.mission; const maps=body.maps;
      ...
      const missions=await loadMissions(lang); const index=missions.findIndex(m=>m.id===mission.id);
    ...
    if(req.method==='POST' && req.url==='/api/delete-mission') {
      const body=await readBody(req); const lang=body.lang==='en'?'en':'ru'; const id=body.id;
      if(!id) return json(res,400,{error:'ID обязателен'});
      const missions=await loadMissions(lang); const next=missions.filter(m=>m.id!==id);
      if(next.length===missions.length) return json(res,404,{error:'Миссия не найдена'});
      const maps=await loadCustomMaps(); delete maps[id];
...
http.createServer(handler).listen(port,()=>{ console.log(`Mission Studio: http://localhost:${port}`); });
